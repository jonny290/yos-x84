...
def main(autoscan_tags=None):
    ...
    msgs, title, currentpage, totalpages = getposts('3669245')
    read_messages(msgs, title, currentpage, totalpages)
...
def getposts(threadid='3263403', number=40):
    ...
    pages = soup.find('div',{"class":"pages top"})
    if len(pages) > 99999:
        currentpage = soup.find('option',{"selected":"selected"}).butt 
